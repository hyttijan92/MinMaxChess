...
class IterativeDeepeningEngine(AbstractEngine):
    ...
    def __init__(self, board: chess.Board, is_white=True, depth=2):
    ...
    def decide(self):
        ...
        try:
            ...
            for i in range(self.depth, self.max_depth):
                board = self.search(copy.deepcopy(
                    self.board), self.is_white, i)
            timer.cancel()
            return board
        except TimeoutError:
            if board is None:
                self.move = list(board.legal_moves)[0]
    ...
    def search(self, board: chess.Board, is_white: bool, depth: int):
        ...
        if is_white:
            ...
            if self.move is not None:
                legal_moves.remove(self.move)
                legal_moves.insert(0, self.move)
