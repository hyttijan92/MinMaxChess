...
class IterativeDeepeningEngine(AbstractEngine):
    ...
    def decide(self):
        ...
        for i in range(self.depth, self.max_depth):
            prev_board = board           
            agent = AlphaBetaEngine(copy.deepcopy(self.board), self.is_white, i, start)
            board = agent.decide()
            if agent.timeout and prev_board is not None:
                return prev_board
        return board
    ...
    def decide(self):
        ...
        if self.is_white:
            ...
            for move in legal_moves:
                ...
                if value > max_value or max_move is None:
                    max_value = value
                    max_move = move
                self.board.pop()
                if max_value > beta:
            ...
            for move in legal_moves:
                ...
                if value < min_value or min_move is None:
                    min_value = value
                    min_move = move
                self.board.pop()
                if min_value < alpha:
